...
class JI_Database {
    ...
    */
    ...
    /**
    * Opções após SELECT, INSERT, UPDATE or DELETE
    * @access protected
    * @var string
    */
    ...
    */
    ...
    /**
    * armazena os dados dos parametros da condição (where)
    * @access protected
    * @var array
    */
    ...
    /**
    * armazena o total de linhas encontradas no banco em um select
    * @var string
    */ 
    public $count = 0;
    
    /**
    * Armazena o total de linhas no metodo withTotalCount()
    * @var string
    */ 
    public $totalCount = 0;
    ...
    */
    ...
    /**
    * Se a SubQuery é um objeto
    * @access protected
    */
    protected $isSubQuery = false;

    /**
    * Variaveis que armazenam informações da query
    * @access protected
    */
    ...
    */
    private function __construct()  {
        ...
        //Config
        $param = Config::getConfiguration('database');  

        //Check
        $check = array('host', 'database', 'username', 'port', 'charset');
        foreach ($check as $key) {
            if (empty($param[$key])) {
                $error = Language::getMessage('database', 'database_config_error', array('field' => $key));
                Log::message($error, array(1, 2));
                JI_ERRORS::generalError($error);
            }    
        }
    ...
    */
    public function connect() {
        ...
        try {
            ...
            Log::message(mysqli_connect_error(), array(1, 2));
            JI_ERRORS::generalError(mysqli_connect_error());     
        }
    ...
    public static function getInstance() {
        if (self::$_instance == null)
            self::$_instance = new JI_Database();
     ...
     */
    public function countAll($tableName, $columns = '*') {
        $res = $this->getOne($tableName, 'count(1) as total');
     ...
     * @uses $MySqliDb->where("firstname", Array ('LIKE' => '%John%')) 
     * @uses $MySqliDb->where("id", Array('1','2','3'), 'IN')
     * @uses $MySqliDb->where ("id", Array('2','3'), 'between')
     * @uses $MySqliDb->where("createdAt", Array (">" => 12));
     * @uses $MySqliDb->where("id = ? or id = ?", Array(1,2))
    ...
    protected function _prepareQuery() {
        if (!$stmt = $this->_mysqli->prepare($this->_query)) {
            $error = Language::getMessage('database', 'database_prepare_query', array('query' => $this->_query, 'error' => $this->_mysqli->error));
            Log::message($error, array(1, 2));
            JI_ERRORS::generalError($error);
